...
model = solow()
model.eval()
model.transpath(T=T,A1=1.5,t0=t0)
##########################################
...
ax1.grid()
ax1.set_ylim(np.min(np.min(model.ktil_trans)-0.25,0), np.max(model.ktil_trans)+0.25)
ax1.set_xlim(0, T)
...
ax2.grid()
ax2.set_ylim(np.min(np.min(model.ytil_trans)-0.25,0), np.max(model.ytil_trans)+0.25)
ax2.set_xlim(0, T)
...
ax3.grid()
ax3.set_ylim(np.min(np.min(model.ctil_trans)-0.25,0), np.max(model.ctil_trans)+0.25)
ax3.set_xlim(0, T)
...
ax4.grid()
ax4.set_ylim(np.min(np.min(model.itil_trans)-0.25,0), np.max(model.itil_trans)+0.25)
ax4.set_xlim(0, T)
ax4.set_title('Investment $i_t$',fontsize=20)
ax4.text(T-23, np.min([np.min(model.itil_trans)-0.2,0+0.5]), 'Created by Brian C. Jenkins',
         fontsize=11, color='black',alpha=0.5)
...
ani.save('../video/increase_tfp.mp4',writer=writer)
